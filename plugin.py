# -*- coding: utf-8 -*-
"""HumanThinking Plugin for QwenPaw v1.1.5.post2+"""
...
async def _init_and_stop_agent(mgr, name: str) -> None:
    """初始化单个 agent 的数据库然后安全关闭"""
    try:
        await mgr.start()
        logger.info(f"Initialized DB for agent '{name}': {mgr.db_path}")
        await mgr.stop()
    except Exception as e:
        logger.warning(f"Failed to init/stop DB for agent '{name}': {e}")
    ...
    def _find_qwenpaw_root(self) -> str:
        try:
            ...
            if qwenpaw_file:
                ...
                while path != os.path.sep and path != '':
        ...
        try:
            ...
            if HumanThinkingMemoryManager is not None:
                try:
                    ...
                    # Docker/多环境：为所有已有 agent 初始化数据库
                    existing_workspaces = _resolve_all_agent_workspace_dirs()
                    for agent_ws in existing_workspaces:
                        agent_name = agent_ws.name
                        if agent_name == "default":
                            continue
                        try:
                            agent_db_path = _resolve_agent_workspace_dir(agent_name) / "memory" / f"human_thinking_memory_{agent_name}.db"
                            agent_db_path.parent.mkdir(parents=True, exist_ok=True)
                            agent_mgr = HumanThinkingMemoryManager(
                                working_dir=str(_resolve_agent_workspace_dir(agent_name)),
                                agent_id=agent_name,
                                user_id=None
                            )
                            try:
                                ...
                                if loop.is_running():
                                    _asyncio2.ensure_future(_init_and_stop_agent(agent_mgr, agent_name))
                                else:
                                    loop.run_until_complete(_init_and_stop_agent(agent_mgr, agent_name))
                            except RuntimeError:
                                import asyncio as _asyncio2
            ...
            deep_sleep_minutes = sleep_config.get("deep_sleep_minutes", 120)
            auto_consolidate = sleep_config.get("auto_consolidate", True)
            consolidate_days = sleep_config.get("consolidate_days", 1)
            ...
            try:
                ...
                if loop.is_running():
                    asyncio.ensure_future(sleep_mgr.start())
                else:
                    loop.run_until_complete(sleep_mgr.start())
            except RuntimeError:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                loop.run_until_complete(sleep_mgr.start())
            logger.info("SleepManager initialized and background loop started")
        ...
        try:
            ...
            try:
                ...
                if sm:
                    ...
                    try:
                        _loop2 = _asy.get_running_loop()
                    except RuntimeError:
                        _loop2 = _asy.new_event_loop()
                        _asy.set_event_loop(_loop2)
                    _loop2.run_until_complete(sm.stop())
