    ...
    def test_get_stats(self, client):
        ...
        with patch('api.routes.HumanThinkingDB') as mock_db_class:
            ...
            mock_db.get_stats = MagicMock(return_value={
                ...
                'memories_last_24h': 5,
                'memories_last_7d': 20,
                'memories_last_30d': 50,
                'tier_distribution': {'short_term': 50, 'long_term': 30},
                'type_distribution': {'general': 60, 'fact': 20},
                'importance_distribution': {'3': 50, '4': 20},
                'sleep_cycles_total': 2,
                'memories_consolidated_total': 15,
            ...
            assert data["active_memories"] == 50
            assert data["frozen_memories"] == 10
            assert data["active_sessions"] == 3
            assert data["emotional_records"] == 3
...
class TestHelperFunctions:
    ...
    def test_get_sleep_manager_singleton(self):
        """测试睡眠管理器单例"""
        from HumanThinking.api.routes import get_sleep_manager
    ...
    def test_get_memory_manager_lru(self):
        """测试记忆管理器 LRU 清理"""
        from HumanThinking.api.routes import get_memory_manager, _MAX_MEMORY_MANAGERS
    ...
    def test_handle_api_errors_decorator_exists(self):
        """测试错误处理装饰器存在"""
        from HumanThinking.api.routes import handle_api_errors
...
class TestRequestModels:
    ...
    def test_search_request_valid(self):
        """测试有效搜索请求"""
        from HumanThinking.api.routes import SearchRequest
    ...
    def test_memory_update_request(self):
        """测试记忆更新请求"""
        from HumanThinking.api.routes import MemoryUpdateRequest
    ...
    def test_sleep_config_update_request(self):
        """测试睡眠配置更新请求"""
        from HumanThinking.api.routes import SleepConfigUpdateRequest
    ...
    def test_force_sleep_request(self):
        """测试强制睡眠请求"""
        from HumanThinking.api.routes import ForceSleepRequest
    ...
    def test_config_update_request(self):
        """测试配置更新请求"""
        from HumanThinking.api.routes import ConfigUpdateRequest
