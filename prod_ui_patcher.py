...
def find_qwenpaw_console_static_dir(qwenpaw_root: str) -> str:
    ...
    # 方法3: common paths（包venv 中的 site-packages）
    candidates = [
    ...
    # 系统级路径（glob 动态发现 Python 版本）
    import glob as _glob
    for prefix in ["/usr/local/lib", "/usr/lib"]:
        for python_dir in sorted(_glob.glob(f"{prefix}/python3.*"), reverse=True):
            candidates.append(str(Path(python_dir) / "site-packages" / "qwenpaw" / "console"))
...
def patch_js_file(filepath: str) -> bool:
    ...
    try:
        ...
        # ============================================
        # 模式 A0: Ta=/Ts= 等后端映射格式 (新版 QwenPaw bundled JS)
        # v1.1.5.post1: Ta={remelight:{...,component:Ca,...}}
        # v1.1.5.post2: Ts={remelight:{...,component:Cs,...}}
        # 通用格式: XXX={remelight:{...}} -> Object.entries(XXX).map(...) 生成下拉菜单
        # ============================================
        ta_match = re.search(r"\w+\s*=\s*\{[^}]*remelight[^}]*\}", content)
        if ta_match:
            original_ta = ta_match.group(0)
            var_name = re.match(r"(\w+)", original_ta).group(1)
            logger.info(f"  ✓ Detected backend mapping ({var_name}= format) in {os.path.basename(filepath)}")
            ...
            # 在 remelight 条目后插入 human_thinking 条目
            new_ta = re.sub(
                r'(remelight:\{[^}]*\})',
                rf'\1{ht_entry}',
                original_ta,
            ...
            if new_ta != original_ta:
                content = content.replace(original_ta, new_ta, 1)
                if "human_thinking" not in content:
                    logger.error(f"✗ Failed to inject human_thinking into {var_name}=")
                    return False
                logger.info(f"  ✓ Patched {var_name}= mapping with human_thinking in {os.path.basename(filepath)}")
                with open(filepath, "w", encoding="utf-8") as f:
                    f.write(content)
                logger.info(f"✓ Successfully patched {var_name}= format: {os.path.basename(filepath)}")
                return True
            else:
                logger.warning(f"  ✗ {var_name}= substitution produced no change, trying fallback...")
                # fallback: 直接用字符串替换
                fallback_new = original_ta.rstrip("}") + ht_entry + "}"
                if fallback_new != original_ta:
                    content = content.replace(original_ta, fallback_new)
                    if "human_thinking" in content:
                        logger.info(f"  ✓ Fallback {var_name}= patching succeeded")
                        with open(filepath, "w", encoding="utf-8") as f:
                            f.write(content)
                        logger.info(f"✓ Successfully patched {var_name}= format (fallback): {os.path.basename(filepath)}")
                        return True
...
def patch_index_html(qwenpaw_root: str, plugin_dir: str = None) -> dict:
    ...
    try:
        ...
        # 始终复制最新的 frontend.js 到 assets 目录
        ...
        if 'frontend.js' in content:
            logger.info("[patch_index_html] HTML already injected")
...
def patch_human_thinking_config_tab(qwenpaw_root: str) -> dict:
    ...
    # 查找包含 memory tab 生成逻辑的文件（不单纯选最大的文件）
    # v1.1.5.post2 中 vendor JS 比 index JS 大很多但不包含 remelight
    target_file = None
    target_size = 0
    
    for js_file in js_files:
        try:
            # 先快速检查文件是否包含 memory_backend 或 remelight
            file_size = os.path.getsize(js_file)
            if file_size < 50000:
                continue
            with open(js_file, "r", encoding="utf-8", errors="ignore") as f:
                preview = f.read(10000)
            if "remelight" not in preview and "memory_manager_backend" not in preview:
                continue
    ...
    if not target_file or target_size < 100000:
        results["errors"].append(f"No suitable JS file found (largest: {target_size} bytes)")
        return results
    
    logger.info(f"[HTConfigTab] Target file: {os.path.basename(target_file)} ({target_size} bytes)")
    ...
    try:
        ...
        if "HumanThinking: HT记忆配置 tab" in content:
            logger.info(f"[HTConfigTab] Already patched: {os.path.basename(target_file)}")
            results["success"] = True
            return results
        
        # v1.1.5.post2 tab injection: const S=Ts[T];if(S){...I.push({key:S.tabKey,...})}
        # v1.1.5.post1: const v=xa[T];if(v){const j=v.component;R.push({key:v.tabKey,...})}
        memory_mapping_patterns = [
            # v1.1.5.post2: const S=Ts[T];if(S){const C=S.component;I.push(...)}
            r'(const\s+\w+=\w+\[\w+\];if\(\w+\)\{const\s+\w+=\w+\.component;\w+\.push\(\{key:\w+\.tabKey,label:[^}]+\},children:[^}]+\}\)\})',
            # v1.1.4+: \w+\.push\(\{key:\w+\.tabKey,label:[^}]*agentConfig\.\$\{[^}]*tabKey\}Title[^}]*\},children:[^}]+\}\)\)
            r'(\w+\.push\(\{key:\w+\.tabKey,label:[^}]*agentConfig\.\$\{[^}]*tabKey\}Title[^}]*\},children:[^}]+\}\)\})',
            # 最宽松模式
            r'(\w+\.push\(\{key:\w+\.tabKey,label:[^}]+\},children:[^}]+\}\)\})',
        ]
        
        patched = False
        for pattern in memory_mapping_patterns:
            # 收集所有匹配，取最后一个（最可能匹配到 memory tab push）
            matches = list(re.finditer(pattern, content))
            if not matches:
                continue
            
            match = matches[-1]  # 取最后一个 match
            logger.info(f"[HTConfigTab] Found memory tab push code (pattern matched, {len(matches)} occurrences)")
...
(function(){
    ...
    var _checkInterval = setInterval(function(){
        _checkCount++;
        if (_checkCount > 60) { clearInterval(_checkInterval); return; }
        try {
            var storage = sessionStorage.getItem('qwenpaw-agent-storage');
            if (!storage) return;
            var data = JSON.parse(storage);
            var agentId = data.state?.selectedAgent || 'remelight';
            if (agentId === 'human_thinking') {
...
'''
            ...
            if new_content != content:
                _make_backup(target_file)
                with open(target_file, "w", encoding="utf-8") as f:
                    f.write(new_content)
                results["patched"].append(os.path.basename(target_file))
                results["success"] = True
                patched = True
                logger.info(f"[HTConfigTab] ✓ Patched: {os.path.basename(target_file)}")
                break
        
        if not patched:
            logger.warning(f"[HTConfigTab] Could not find memory tab push pattern in {os.path.basename(target_file)}")
...
(function(){
    // 先从sessionStorage读取当前agent，避免首次加载时触发
    var storageRaw = sessionStorage.getItem("qwenpaw-agent-storage");
    var lastAgent = null;
    if (storageRaw) {
        try {
            var parsed = JSON.parse(storageRaw);
            lastAgent = parsed.state?.selectedAgent || null;
        } catch(e) {}
    }
    var checkInterval = setInterval(function(){
        try {
            ...
            if (currentAgent && currentAgent !== lastAgent && lastAgent !== null) {
                lastAgent = currentAgent;
                console.log("[HumanThinking] Agent switched to:", currentAgent, "- triggering __agentConfigFetch...");
                if (window.__agentConfigFetch) {
                    window.__agentConfigFetch();
                } else {
                    console.warn("[HumanThinking] __agentConfigFetch not ready, skipping reload");
                }
            }
        } catch(e) {}
    }, 500);
...
def inject_persistent_config() -> dict:
    ...
    env = detect_qwenpaw_env()
    config_path = os.path.join(env.working_dir, "config.json") if env.working_dir else os.path.expanduser("~/.qwenpaw/config.json")
    ...
    working_dir = env.working_dir or str(Path.home() / ".qwenpaw")
    agent_json_pattern = os.path.join(working_dir, "workspaces", "*", "agent.json")
    if _DEBUG:
        _log(f"inject_persistent_config: agent_json_pattern={agent_json_pattern}")
...
"""
    ...
    # 动态发现 venv 中所有 Python 版本的 site-packages
    import glob as _glob
    env = detect_qwenpaw_env()
    venv_lib = os.path.join(env.venv_dir, "lib") if env.venv_dir else None
    if venv_lib:
        for python_dir in sorted(_glob.glob(f"{venv_lib}/python3.*"), reverse=True):
            candidates.append(os.path.join(python_dir, "site-packages", "sitecustomize.py"))
...
def patch_plugins_router(qwenpaw_root: str) -> dict:
    ...
    try:
        ...
        import glob as _glob
        for python_dir in sorted(_glob.glob(os.path.join(qwenpaw_root, "venv", "lib", "python3.*")), reverse=True):
            search_paths.append(os.path.join(python_dir, "site-packages", "qwenpaw", "app", "routers", "plugins.py"))
...
def _find_runner_py() -> str:
    """动态查找 QwenPaw runner.py 路径"""
    import os
    
    try:
        import qwenpaw
        pkg_dir = os.path.dirname(qwenpaw.__file__)
        runner_path = os.path.join(pkg_dir, "app", "runner", "runner.py")
    ...
    # 优先用 env_detector 动态发现 venv
    try:
        env = detect_qwenpaw_env()
        if env.venv_dir:
            venv_lib = os.path.join(env.venv_dir, "lib")
            import glob as _glob2
            for python_dir in sorted(_glob2.glob(f"{venv_lib}/python3.*"), reverse=True):
                candidate = os.path.join(python_dir, "site-packages", "qwenpaw", "app", "runner", "runner.py")
    ...
    import glob as _glob
    # Docker 自主发现
    if os.path.exists("/app/venv/lib"):
        for python_dir in sorted(_glob.glob("/app/venv/lib/python3.*"), reverse=True):
            candidate = os.path.join(python_dir, "site-packages", "qwenpaw", "app", "runner", "runner.py")
            if candidate not in candidates:
                candidates.insert(0, candidate)
    # 本地 ~/.qwenpaw
    home_venv = os.path.join(os.path.expanduser("~"), ".qwenpaw", "venv", "lib")
    if os.path.exists(home_venv):
        for python_dir in sorted(_glob.glob(f"{home_venv}/python3.*"), reverse=True):
            candidate = os.path.join(python_dir, "site-packages", "qwenpaw", "app", "runner", "runner.py")
            if candidate not in candidates:
                candidates.insert(0, candidate)
    ...
    try:
        ...
        if env.qwenpaw_package_dir:
            runner_path = os.path.join(str(env.qwenpaw_package_dir), "app", "runner", "runner.py")
            if os.path.isfile(runner_path):
                return runner_path
    except Exception:
        pass
...
def inject_runner_post_reply_dispatch() -> dict:
    ...
    修复策略 v2：将 post_reply 放入 finally 块确保必定执行。
    同时保留 try 块中的正常路径调用，使用 _post_reply_done 标志防止重复。
    
    覆盖所有退出场景：
    - 正常完成 → try 块执行 post_reply，finally 检查标志跳过
    - Agent 异常 → finally 块兜底执行 post_reply
    - GeneratorExit/aclose → finally 块兜底执行 post_reply
    - CancelledError → finally 块兜底执行 post_reply
    """
    import os
    
    runner_path = _find_runner_py()
    if not runner_path or not os.path.isfile(runner_path):
        return {"success": False, "error": f"runner.py not found"}
    
    try:
        with open(runner_path, "r") as f:
            content = f.read()
        
        new_version_marker = "# ── HumanThinking: memory flush [BEGIN]"
        if new_version_marker in content:
            logger.info("[PostReplyDispatch] Already injected (v3 with memory flush)")
            return {"success": True, "injected": False, "message": "Already injected (v3 with memory flush)"}
        
        if "# ── HumanThinking: post_reply finally dispatch [BEGIN]" in content:
            logger.info("[PostReplyDispatch] Old v2 injection found, restoring from backup...")
            backup_path = runner_path + ".humanthinking.bak"
            if os.path.exists(backup_path):
                shutil.copy2(backup_path, runner_path)
                with open(runner_path, "r") as f:
                    content = f.read()
                logger.info("[PostReplyDispatch] Restored from backup")
            else:
                pattern_v2_finally = r'\n\s*# ── HumanThinking: post_reply finally dispatch \[BEGIN\] ──.*?# ── HumanThinking: post_reply finally dispatch \[END\] ──\s*\n'
                content = re.sub(pattern_v2_finally, '\n', content, flags=re.DOTALL)
                flag_v2_old = "        _post_reply_done = False\n        try:\n            session_id = request.session_id"
                flag_v2_new = "        try:\n            session_id = request.session_id"
                if flag_v2_old in content:
                    content = content.replace(flag_v2_old, flag_v2_new, 1)
                pattern_v2_try = r'\n\s*# ── HumanThinking: post_reply dispatch \[BEGIN\] ──.*?# ── HumanThinking: post_reply dispatch \[END\] ──\n'
                content = re.sub(pattern_v2_try, '\n', content, flags=re.DOTALL)
                logger.info("[PostReplyDispatch] Removed old v2 injection manually")
        
        old_begin = "# ── HumanThinking: post_reply dispatch [BEGIN]"
        if old_begin in content:
            logger.info("[PostReplyDispatch] Old v1 injection found, restoring from backup...")
            backup_path = runner_path + ".humanthinking.bak"
            if os.path.exists(backup_path):
                shutil.copy2(backup_path, runner_path)
                with open(runner_path, "r") as f:
                ...
                logger.info("[PostReplyDispatch] Removed old v1 injection manually")
        
        _make_backup(runner_path)
        
        flag_old = "        try:\n            session_id = request.session_id"
        flag_new = "        _post_reply_done = False\n        try:\n            session_id = request.session_id"
        
        if flag_old not in content:
            return {"success": False, "error": "Could not find flag injection point (try: block) in runner.py"}
        content = content.replace(flag_old, flag_new, 1)
        
        try_old = "                    yield msg, last\n\n        except asyncio.CancelledError as exc:"
        try_injection = """
            ...
            if self.context_manager is not None:
                try:
                    ...
                    _post_reply_done = True
                    logger.info("post_reply completed via try block")
                except Exception:
                    logger.warning("post_reply hook failed in try block", exc_info=True)
            # ── HumanThinking: post_reply dispatch [END] ──

            # ── HumanThinking: memory store [BEGIN] ──
            try:
                _mm = getattr(agent, 'memory_manager', None) if agent is not None else None
                if _mm is not None and hasattr(_mm, 'store_memory') and hasattr(agent, 'memory') and agent.memory is not None:
                    _recent = agent.memory.content[-6:] if len(agent.memory.content) >= 6 else agent.memory.content
                    _parts = []
                    for _m, __ in _recent:
                        _c = _m.content if hasattr(_m, 'content') else str(_m)
                        if isinstance(_c, list):
                            _c = ' '.join(str(x) for x in _c if x)
                        if _c and isinstance(_c, str):
                            _parts.append(_c[:500])
                    if _parts:
                        await _mm.store_memory(
                            content=chr(10).join(_parts)[:2000],
                            session_id=session_id,
                            user_id=user_id,
                            role='assistant',
                            importance=3,
                            memory_type='conversation',
                            metadata={'source': 'turn_end', 'msg_count': len(_parts)}
                        )
            except Exception:
                pass
            # ── HumanThinking: memory store [END] ──

            # ── HumanThinking: memory flush [BEGIN] ──
            if agent is not None:
                mm = getattr(agent, 'memory_manager', None)
                if mm is not None and hasattr(mm, 'post_memory_operation'):
                    try:
                        ...
                        logger.info("memory flush completed via try block")
                    except Exception:
                        logger.warning("memory flush failed in try block", exc_info=True)
            # ── HumanThinking: memory flush [END] ──
"""
        try_new = "                    yield msg, last\n" + try_injection + "\n        except asyncio.CancelledError as exc:"
        
        if try_old not in content:
            return {"success": False, "error": "Could not find try injection point (yield msg, last) in runner.py"}
        content = content.replace(try_old, try_new, 1)
        
        finally_old = "        finally:\n            if agent is not None and session_state_loaded:"
        finally_injection = """        finally:
            # ── HumanThinking: post_reply finally dispatch [BEGIN] ──
            if not _post_reply_done and self.context_manager is not None:
                try:
                    await self.context_manager.post_reply(
                        agent=agent,
                        kwargs={},
                    ...
                    logger.info("post_reply completed via finally block")
                except Exception:
                    logger.warning("post_reply hook failed in finally", exc_info=True)
            # ── HumanThinking: post_reply finally dispatch [END] ──
            ...
            if agent is not None:
                ...
                if mm is not None and hasattr(mm, 'post_memory_operation'):
                    try:
                        ...
                        logger.info("memory flush completed via finally block")
                    except Exception:
                        logger.warning("memory flush failed in finally", exc_info=True)
            # ── HumanThinking: memory flush [END] ──

            if agent is not None and session_state_loaded:"""
        
        if finally_old not in content:
            return {"success": False, "error": "Could not find finally injection point in runner.py"}
        content = content.replace(finally_old, finally_injection, 1)
        
        if "_post_reply_done = False" not in content:
            return {"success": False, "error": "Flag variable not injected"}
        if "post_reply completed via try block" not in content:
            return {"success": False, "error": "Try block post_reply not injected"}
        if "post_reply completed via finally block" not in content:
            return {"success": False, "error": "Finally block post_reply not injected"}
        if "memory flush completed via try block" not in content:
            return {"success": False, "error": "Try block memory flush not injected"}
        if "memory flush completed via finally block" not in content:
            return {"success": False, "error": "Finally block memory flush not injected"}
        
        with open(runner_path, "w") as f:
            f.write(content)
        
        logger.info("[PostReplyDispatch] Injected v3 post_reply + memory flush dispatch (try + finally)")
