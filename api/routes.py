...
def _get_agent_json_path(agent_id: str) -> str:
    return str(resolve_agent_workspace_dir(agent_id) / "agent.json")
...
def _write_qwenpaw_auto_memory_interval(agent_id: str, value: int) -> bool:
    ...
    try:
        ...
        dir_name = os.path.dirname(agent_json_path)
        try:
...
def _get_feishu_agent_id() -> Optional[str]:
    ...
    agent_files = glob_mod.glob(str(resolve_qwenpaw_dir() / "workspaces" / "*" / "agent.json"))
...
class StatsResponse(BaseModel):
    ...
    insight_count: int = 0
    dream_log_count: int = 0
    emotional_records: int = 0
    ...
    sleep_cycles_total: int = 0
    memories_consolidated_total: int = 0
...
def get_memory_manager(agent_id: str = None):
    ...
    import threading
    _create_lock = getattr(get_memory_manager, '_lock', None)
    if _create_lock is None:
        _create_lock = threading.Lock()
        get_memory_manager._lock = _create_lock
...
async def get_stats(
    ...
    return StatsResponse(
        ...
        dream_log_count=stats.get('dream_log_count', 0),
        emotional_records=stats.get('emotional_records', 0),
        memories_last_24h=stats.get('memories_last_24h', 0),
        memories_last_7d=stats.get('memories_last_7d', 0),
        memories_last_30d=stats.get('memories_last_30d', 0),
        ...
        sleep_cycles_total=stats.get('sleep_cycles_total', 0),
        memories_consolidated_total=stats.get('memories_consolidated_total', 0),
        tokens_saved_total=stats.get('tokens_saved_total', 0),
    )
