...
class EmotionalContinuityEngine:
    ...
    async def track_emotional_state(
        ...
        await self.db.insert_emotional_state(
            session_id=session_id,
            agent_id=agent_id,
            user_id=user_id,
            emotional_state_json=json.dumps(emotional_data),
    ...
    async def get_emotional_context(
        ...
        if not recent_states:
            rows = await self.db.get_recent_emotional_states(agent_id, user_id, 5)
            recent_states = [
                {
    ...
    async def calculate_emotional_continuity(
        ...
        rows = await self.db.get_emotional_states_for_continuity(agent_id, user_id, 3)
        if len(rows) < 2:
            return 0.0
