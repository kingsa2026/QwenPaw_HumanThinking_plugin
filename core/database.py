...
class HumanThinkingDB:
    ...
    async def initialize(self) -> None:
        ...
        is_docker = os.environ.get('QWENPAW_RUNNING_IN_CONTAINER') == '1' or bool(os.environ.get('QWENPAW_WORKING_DIR', '').startswith('/app'))
        
        # 启用 WAL 模式（Docker 卷可能不支持，需要 fallback）
        wal_ok = False
        try:
            ...
            if row and row[0].upper() == 'WAL':
                wal_ok = True
                logger.info(f"WAL mode enabled on {self.db_path}")
        except sqlite3.OperationalError as e:
            logger.warning(f"WAL mode failed: {e}, falling back to DELETE mode")
        
        if not wal_ok:
            try:
                self.cursor.execute("PRAGMA journal_mode=DELETE")
                logger.info("Fallback to DELETE journal mode")
        ...
        try:
            ...
            logger.debug("mmap_size PRAGMA not supported on this platform")
        self.conn.commit()
        
        if is_docker:
            logger.info(f"Docker env detected: SQLite={sqlite_ver}, WAL={'OK' if wal_ok else 'fallback'}, db={self.db_path}")
    ...
    async def _create_tables(self) -> None:
        ...
        self.cursor.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS qwenpaw_memory (
                ...
                -- ========== 新增：记忆合并 ==========
                last_consolidated_at DATETIME,     -- 上次巩固时间
        ...
        try:
            ...
            logger.warning("FTS5 not available in this SQLite build, falling back to LIKE search")
    ...
    async def get_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            self.cursor.execute(
                "SELECT COUNT(*) FROM qwenpaw_memory_archive WHERE agent_id = ?",
                (agent_id,)
            )
            stats["archived_memories"] = self.cursor.fetchone()[0]
        except Exception:
            stats["archived_memories"] = 0
        
        try:
            self.cursor.execute(
                "SELECT COUNT(*) FROM humanthinking_insights WHERE agent_id = ?",
                (agent_id,)
            )
            stats["insight_count"] = self.cursor.fetchone()[0]
        except Exception:
            stats["insight_count"] = 0
        
        try:
            self.cursor.execute(
                "SELECT COUNT(*) FROM humanthinking_dream_logs WHERE agent_id = ?",
                (agent_id,)
            )
            stats["dream_log_count"] = self.cursor.fetchone()[0]
        except Exception:
            stats["dream_log_count"] = 0
        
        try:
            self.cursor.execute(
                "SELECT COUNT(*) FROM session_emotional_continuity WHERE agent_id = ?",
                (agent_id,)
            )
            stats["emotional_records"] = self.cursor.fetchone()[0]
        except Exception:
            stats["emotional_records"] = 0
        
        self.cursor.execute("""
            SELECT COUNT(*) FROM qwenpaw_memory
            WHERE agent_id = ? AND deleted_at IS NULL AND created_at > datetime('now', '-1 day')
        """, (agent_id,))
        stats["memories_last_24h"] = self.cursor.fetchone()[0]
        
        self.cursor.execute("""
            SELECT COUNT(*) FROM qwenpaw_memory
            WHERE agent_id = ? AND deleted_at IS NULL AND created_at > datetime('now', '-7 days')
        """, (agent_id,))
        stats["memories_last_7d"] = self.cursor.fetchone()[0]
        
        self.cursor.execute("""
            SELECT COUNT(*) FROM qwenpaw_memory
            WHERE agent_id = ? AND deleted_at IS NULL AND created_at > datetime('now', '-30 days')
        """, (agent_id,))
        stats["memories_last_30d"] = self.cursor.fetchone()[0]
        ...
        stats["tier_distribution"] = {
            str(row["memory_tier"] or "unknown"): row["count"] 
            for row in self.cursor.fetchall()
        }
        ...
        stats["type_distribution"] = {
            str(row["memory_type"] or "unknown"): row["count"] 
            for row in self.cursor.fetchall()
        }
        ...
        try:
            self.cursor.execute("""
                SELECT COUNT(DISTINCT session_id) FROM memory_working_cache
                WHERE agent_id = ? AND expires_at IS NULL
            """, (agent_id,))
            active_sessions_from_cache = self.cursor.fetchone()[0]
        except Exception:
            active_sessions_from_cache = 0
        
        stats["active_sessions"] = max(stats["total_sessions"], active_sessions_from_cache)
        
        try:
            self.cursor.execute("""
                SELECT COUNT(*) as total,
                       SUM(memories_consolidated) as consolidated,
                       SUM(tokens_saved) as tokens
                FROM humanthinking_dream_logs
                WHERE agent_id = ?
            """, (agent_id,))
            row = self.cursor.fetchone()
            stats["sleep_cycles_total"] = row["total"] or 0
            stats["memories_consolidated_total"] = row["consolidated"] or 0
            stats["tokens_saved_total"] = row["tokens"] or 0
        except Exception:
            stats["sleep_cycles_total"] = 0
            stats["memories_consolidated_total"] = 0
            stats["tokens_saved_total"] = 0
    ...
    async def insert_emotional_state(
        self,
        session_id: str,
        agent_id: str,
        user_id: Optional[str] = None,
        emotional_state_json: str = "{}",
        ...
        self.cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (session_id, agent_id, user_id, emotional_state_json, continuity_score))
        self.conn.commit()

    async def get_recent_emotional_states(
        self,
        agent_id: str,
        user_id: Optional[str] = None,
        limit: int = 5
        ...
        self.cursor.execute("""
            ...
            ORDER BY created_at DESC
            LIMIT ?
        """, (agent_id, user_id, user_id, limit))
        return self.cursor.fetchall()

    async def get_emotional_states_for_continuity(
        self,
        agent_id: str,
        user_id: Optional[str] = None,
        limit: int = 3
