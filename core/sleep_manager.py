...
class SleepManager:
    ...
    async def start(self):
        if self._running:
            return
        self._running = True
        self._background_task = asyncio.create_task(self._background_sleep_loop())
        logger.info("SleepManager background loop started")
    ...
    async def stop(self):
        ...
        if self._background_task:
            ...
            try:
                ...
                pass
            self._background_task = None
        logger.info("SleepManager background loop stopped")
    
    async def _background_sleep_loop(self):
        while self._running:
            try:
                check_interval = min(
                    self._default_config.light_sleep_seconds or 60,
                    self._default_config.rem_seconds or 120,
                    self._default_config.deep_sleep_seconds or 300
                )
                await asyncio.sleep(max(check_interval, 30))
                if not self._running:
                    break
                for agent_id in list(self._agent_states.keys()):
                    try:
                        await self.check_and_trigger_sleep(agent_id)
                    except Exception:
                        pass
            except asyncio.CancelledError:
                break
    ...
    async def _get_last_activity_time(self, agent_id: str) -> Optional[float]:
        ...
        try:
            ...
            if recent and len(recent) > 0:
                ts = None
                for field in ("created_at", "timestamp", "created", "updated_at"):
                    ts = recent[0].get(field)
                    if ts:
                        break
                if ts:
                    try:
                    ...
                    logger.debug(f"No timestamp field found in memory row for {agent_id}, fields: {list(recent[0].keys())[:5]}")
    ...
    async def _update_sleep_state(self, agent_id: str, state: AgentSleepState, idle_time: float, current_time: float):
        ...
        try:
            ...
            # 如果空闲时间低于浅睡阈值，恢复活跃状态
            if idle_time < config.light_sleep_seconds and not state.is_active and not state.forced_state:
                logger.info(f"Agent {agent_id} waking up to active")
                self._reset_to_active(state)
            
            # 自愈：检测并恢复不一致的状态（所有状态标记均为False）
            if not state.is_active and not state.is_light_sleep and not state.is_rem and not state.is_deep_sleep:
                logger.warning(f"Agent {agent_id} state inconsistent - all flags are False, resetting to active")
                self._reset_to_active(state)
        except Exception as e:
    ...
    async def _enter_light_sleep(self, state: AgentSleepState, current_time: float):
        ...
        if not state._light_sleep_executed:
            try:
                ...
                logger.info(f"Agent {state.agent_id} light sleep tasks completed successfully")
            except Exception as e:
    ...
    async def _enter_rem(self, state: AgentSleepState, current_time: float):
        ...
        if not state._rem_executed:
            try:
                ...
                logger.info(f"Agent {state.agent_id} REM tasks completed successfully")
            except Exception as e:
    ...
    async def _enter_deep_sleep(self, state: AgentSleepState, current_time: float):
        ...
        if not state._deep_sleep_executed:
            try:
                ...
                logger.info(f"Agent {state.agent_id} deep sleep tasks completed successfully")
            except Exception as e:
    ...
    async def _execute_deep_sleep(self, agent_id: str, state: AgentSleepState):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to write MEMORY.md: {e}", exc_info=True)
    ...
    async def _apply_memory_temperature(self, db, agent_id: str, scored_memories: list):
        ...
        """
        try:
            from .memory_temperature import MemoryTemperature
            ...
            logger.warning(f"memory_temperature module not available for {agent_id}")
        except Exception as e:
...
def record_agent_activity(agent_id: str) -> bool:
    ...
    如果Agent正在睡眠中，会唤醒它并执行必要的唤醒任务（如写入MEMORY.md）。
    返回 True 表示 Agent 从睡眠中被唤醒，返回 False 表示 Agent 本就活跃。
    ...
    if not state.is_active:
        ...
        if state.is_deep_sleep:
            ...
            logger.info(f"Agent {agent_id} woke up from REM by new activity")
        elif state.is_light_sleep:
            logger.info(f"Agent {agent_id} woke up from light sleep by new activity")
        _sleep_manager._reset_to_active(state)
        return True
