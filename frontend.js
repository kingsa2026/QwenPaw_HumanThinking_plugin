...
(function() {
    ...
    // 阻止 QwenPaw 的 patch_agent_config_refresh 触发整页刷新
    window.__agentConfigFetch = function() {
        console.log('[HumanThinking] Agent switch detected via __agentConfigFetch');
        // agent.agent_id change handled by React useEffect deps - no reload needed
    };
    ...
    const loadTranslations = async () => {
        ...
        return {
            ...
            stats: {
                totalMemories: 'Total Memories', active: 'Active', frozen: 'Frozen', archived: 'Archived',
                activeSessions: 'Active Sessions', insights: 'Insights', dreamLogs: 'Dream Logs',
                emotionRecords: 'Emotion Records', last24h: 'Last 24h', last7d: 'Last 7d', last30d: 'Last 30d',
                memoryTiers: 'Memory Tiers', memoryTypes: 'Memory Types', importanceDist: 'Importance',
                sleepSummary: 'Sleep Summary', cycles: 'Cycles', consolidated: 'Consolidated', tokensSaved: 'Tokens Saved'
            }
        };
    };
        ...
        const MemoryManagementSidebar = () => {
            ...
            useEffect(() => {
                const fetchData = async () => {
                    const targetAgent = agent.agent_id || '';
                    try {
                        const queryParam = targetAgent ? '?agent_id=' + targetAgent : '';
                        const [statsRes, sessionsRes] = await Promise.all([
                            apiRequest('/stats' + queryParam),
                            apiRequest('/sessions' + queryParam)
                        ]);
                        if (currentAgentIdRef.current !== targetAgent) return;
                        setStats(statsRes);
            ...
            const renderStats = () => {
                ...
                const statItems = [
                    ...
                    { title: t('stats.active', 'Active'), value: stats.active_memories || 0, color: '#52c41a', icon: Icon('link', 20, '#52c41a') },
                    { title: t('stats.frozen', 'Frozen'), value: stats.frozen_memories || 0, color: '#faad14', icon: Icon('snowflake', 20, '#faad14') },
                    { title: t('stats.archived', 'Archived'), value: stats.archived_memories || 0, color: '#13c2c2', icon: Icon('cloud', 20, '#13c2c2') },
                    { title: t('stats.activeSessions', 'Active Sessions'), value: stats.active_sessions || 0, color: '#722ed1', icon: Icon('activity', 20, '#722ed1') },
                    { title: t('stats.insights', 'Insights'), value: stats.insight_count || 0, color: '#eb2f96', icon: Icon('star', 20, '#eb2f96') },
                    { title: t('stats.dreamLogs', 'Dream Logs'), value: stats.dream_log_count || 0, color: '#2f54eb', icon: Icon('moon', 20, '#2f54eb') },
                    { title: t('stats.emotionRecords', 'Emotion Records'), value: stats.emotional_records || 0, color: '#f759ab', icon: Icon('heart', 20, '#f759ab') }
                ];

                var recentItems = [
                    { title: t('stats.last24h', 'Last 24h'), value: stats.memories_last_24h || 0, color: '#52c41a' },
                    { title: t('stats.last7d', 'Last 7d'), value: stats.memories_last_7d || 0, color: '#1890ff' },
                    { title: t('stats.last30d', 'Last 30d'), value: stats.memories_last_30d || 0, color: '#722ed1' }
                ];

                var hasDistributions = Object.keys(stats.tier_distribution || {}).length > 0
                    || Object.keys(stats.type_distribution || {}).length > 0
                    || Object.keys(stats.importance_distribution || {}).length > 0;

                var hasSleep = (stats.sleep_cycles_total || 0) > 0
                    || (stats.memories_consolidated_total || 0) > 0
                    || (stats.tokens_saved_total || 0) > 0;

                var renderDistBar = function(label, value, total, color) {
                    var pct = total > 0 ? Math.round(value / total * 100) : 0;
                    return React.createElement('div', { style: { marginBottom: 6 }, key: label },
                        React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 2, fontSize: 12 } },
                            React.createElement('span', null, label),
                            React.createElement('span', null, value, ' (', pct, '%)')
                        ),
                        React.createElement('div', { style: { background: '#f0f0f0', borderRadius: 4, height: 8, overflow: 'hidden' } },
                            React.createElement('div', { style: { width: pct + '%', height: '100%', background: color || '#1890ff', borderRadius: 4, transition: 'width 0.3s' } })
                        )
                    );
                };

                var total = stats.total_memories || 1;

                var distSection = null;
                if (hasDistributions) {
                    var tierBars = Object.entries(stats.tier_distribution || {}).map(function(e) {
                        return renderDistBar(e[0], e[1], total, '#1890ff');
                    });
                    var typeBars = Object.entries(stats.type_distribution || {}).map(function(e) {
                        return renderDistBar(e[0], e[1], total, '#52c41a');
                    });
                    var impBars = Object.entries(stats.importance_distribution || {}).sort(function(a, b) { return parseInt(a[0]) - parseInt(b[0]); }).map(function(e) {
                        return renderDistBar('Importance ' + e[0], e[1], total, '#722ed1');
                    });

                    distSection = React.createElement('div', { style: { marginTop: 12 } },
                        React.createElement('div', { style: { fontWeight: 'bold', marginBottom: 8, fontSize: 14 } }, t('stats.memoryTiers', 'Memory Tiers')),
                        React.createElement('div', null, tierBars),
                        Object.keys(stats.type_distribution || {}).length > 0
                            ? React.createElement(React.Fragment, null,
                                React.createElement('div', { style: { fontWeight: 'bold', marginBottom: 8, marginTop: 12, fontSize: 14 } }, t('stats.memoryTypes', 'Memory Types')),
                                React.createElement('div', null, typeBars))
                            : null,
                        Object.keys(stats.importance_distribution || {}).length > 0
                            ? React.createElement(React.Fragment, null,
                                React.createElement('div', { style: { fontWeight: 'bold', marginBottom: 8, marginTop: 12, fontSize: 14 } }, t('stats.importanceDist', 'Importance')),
                                React.createElement('div', null, impBars))
                            : null
                    );
                }

                var sleepSection = null;
                if (hasSleep) {
                    sleepSection = React.createElement('div', { style: { marginTop: 12 } },
                        React.createElement('div', { style: { fontWeight: 'bold', marginBottom: 8, fontSize: 14 } }, t('stats.sleepSummary', 'Sleep Summary')),
                        React.createElement(Row, { gutter: [12, 12] },
                            React.createElement(Col, { span: 8 },
                                React.createElement(Statistic, { title: t('stats.cycles', 'Cycles'), value: stats.sleep_cycles_total || 0, valueStyle: { fontSize: 20, color: '#722ed1' } })
                            ),
                            React.createElement(Col, { span: 8 },
                                React.createElement(Statistic, { title: t('stats.consolidated', 'Consolidated'), value: stats.memories_consolidated_total || 0, valueStyle: { fontSize: 20, color: '#52c41a' } })
                            ),
                            React.createElement(Col, { span: 8 },
                                React.createElement(Statistic, { title: t('stats.tokensSaved', 'Tokens Saved'), value: stats.tokens_saved_total || 0, valueStyle: { fontSize: 20, color: '#1890ff' } })
                            )
                        )
                    );
                }
                ...
                return React.createElement('div', { style: { padding: 16 } },
                    React.createElement(Row, { gutter: [12, 12] },
                        statItems.map(function(item, index) {
                    ...
                    React.createElement(Row, { gutter: [12, 12], style: { marginTop: 12 } },
                        recentItems.map(function(item, index) {
                            return React.createElement(Col, { span: 8, key: 'recent-' + index },
                                React.createElement('div', { className: 'ht-stat-card', style: { textAlign: 'center' } },
                                    React.createElement(Statistic, {
                                        title: item.title,
                                        value: item.value,
                                        valueStyle: { color: item.color, fontSize: 20, fontWeight: 'bold' },
                                        style: { marginBottom: 0 }
                                    })
                                )
                            );
            ...
            useEffect(() => {
                const fetchEmotion = async () => {
                    const targetAgent = agent.agent_id || 'default';
                    try {
                        const currentAgent = getCurrentAgent();
                        const aid = agent.agent_id || (currentAgent && currentAgent.agent_id) || 'default';
                        const data = await apiRequest('/emotion?agent_id=' + aid);
                        if (currentAgentIdRef.current !== targetAgent) return;
                        setEmotion(data);
                    } catch (e) {
                        if (e.name === 'AbortError' || e.message === 'Failed to fetch') return;
                ...
                timer = setTimeout(function() {
                    fetchEmotion();
                    interval = setInterval(fetchEmotion, 5000);
                }, 1500);
                return function() { clearTimeout(timer); clearInterval(interval); };
            }, [agent.agent_id]);
            ...
            useEffect(() => {
                const fetchConfig = async () => {
                    ...
                    try {
                        ...
                        if (currentAgentIdRef.current !== targetAgent) return;
                        setConfig(data);
                        console.log('[HumanThinking] 配置已加载，agent:', targetAgent || 'global');
                    } catch (e) {
                        if (e.name === 'AbortError' || e.message === 'Failed to fetch') return;
                        if (currentAgentIdRef.current !== targetAgent) return;
                ...
                var configTimer, configInterval;
                configTimer = setTimeout(function() {
                    fetchConfig();
                    configInterval = setInterval(fetchConfig, 5000);
                }, 1500);
                ...
                // 同标签页内保底检测（5秒间隔
                var agentCheckInterval = setInterval(function() {
            ...
            useEffect(() => {
                const fetchTimeline = async () => {
                    ...
                    try {
                        ...
                        const data = await apiRequest('/memories/timeline?agent_id=' + aid + '&group_by=' + groupBy);
                        if (currentAgentIdRef.current !== targetAgent) return;
                ...
                var timer = setTimeout(fetchTimeline, 1500);
                    ...
                    React.createElement('div', { style: { marginBottom: 24 } },
                        ...
                        React.createElement('div', {
                            ...
                            React.createElement('div', { style: { marginBottom: 8 } },
                                React.createElement('strong', null, t('about.version', 'Version') + '：'), 'v1.1.5.post2'
                            ),
                            React.createElement('div', { style: { marginBottom: 8 } },
        ...
        const SleepManagementSidebar = () => {
            ...
            const fetchSleepStatus = async () => {
                ...
                try {
                    ...
                    if (sleepCurrentAgentIdRef.current !== targetAgent) return;
                    setSleepStatus(data);
            ...
            useEffect(() => {
                const fetchConfig = async () => {
                    ...
                    try {
                        ...
                        if (sleepCurrentAgentIdRef.current !== targetAgent) return;
                        setSleepConfig(data);
                        console.log('[HumanThinking] 睡眠配置已加载，agent:', targetAgent || 'global');
                    } catch (e) {
                        if (e.name === 'AbortError' || e.message === 'Failed to fetch') return;
                        if (sleepCurrentAgentIdRef.current !== targetAgent) return;
                        ...
                        setSleepConfig({
                            ...
                            auto_memory_interval: 3,
                            feishu_agent_id: null,
                        });
                    }
                };
                var configTimer = setTimeout(fetchConfig, 1500);
    ...
    const init = async () => {
        try {
            React = window.QwenPaw?.host?.React;
            ReactDOM = window.QwenPaw?.host?.ReactDOM;
